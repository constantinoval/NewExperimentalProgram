...
class OptionsForm(QDialog):
    ...
    def accept(self):
        settings = QSettings()
        settings.setValue("pyuic4",
                QVariant(self.pyuic4Label.text()))
        settings.setValue("pyrcc4",
                QVariant(self.pyrcc4Label.text()))
...
class Form(QMainWindow):
    ...
    def __init__(self):
        ...
        self.connect(aboutAction, SIGNAL("triggered()"),
                self.about)
        self.connect(toolsAction, SIGNAL("triggered()"),
                self.setToolPaths)
        self.connect(self.pathButton, SIGNAL("clicked()"),
                self.setPath)
        self.connect(self.buildButton, SIGNAL("clicked()"),
                self.build)
