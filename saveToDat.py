...
import io
import base64
from odbcAccess import unpackTable, tofloat
...
class dat_file(object):
    ...
    diapA = property(partial(read_data, name="diapA"),
                     partial(write_data, name="diapA"))
    diapB = property(partial(read_data, name="diapB"),
                     partial(write_data, name="diapB"))
    tarirA = property(partial(read_data, name="tarirA"),
                      partial(write_data, name="tarirA"))
    tarirB = property(partial(read_data, name="tarirB"),
                      partial(write_data, name="tarirB"))
    E1 = property(partial(read_data, name="E1"),
                  partial(write_data, name="E1"))
    E2 = property(partial(read_data, name="E2"),
                  partial(write_data, name="E2"))
    D1 = property(partial(read_data, name="D1"),
                  partial(write_data, name="D1"))
    D2 = property(partial(read_data, name="D2"),
                  partial(write_data, name="D2"))
    C1 = property(partial(read_data, name="c1"),
                  partial(write_data, name="c1"))
    C2 = property(partial(read_data, name="c2"),
                  partial(write_data, name="c2"))
    S1 = property(partial(read_data, name="S1"),
                  partial(write_data, name="S1"))
    S2 = property(partial(read_data, name="S2"),
                  partial(write_data, name="S2"))
    V0 = property(partial(read_data, name="V0"),
                  partial(write_data, name="V0"))
    ...
    d0 = property(partial(read_data, name="d0"),
                  partial(write_data, name="d0"))
    s = property(partial(read_data, name="s"), partial(write_data, name="s"))
    dt = property(partial(read_data, name="time_to_point"),
                  partial(write_data, name="time_to_point"))
    n1 = property(partial(read_data, name="n1", fmt='h'),
                  partial(write_data, name="n1", fmt='h'))
    n2 = property(partial(read_data, name="n2", fmt='h'),
                  partial(write_data, name="n2", fmt='h'))
...
def saveDAT(datFile, expData, barData):
    #    datFile=datFile+'.dat'
    open(datFile, 'wb').write(base64.decodebytes(sampleFile.read()))
    dat = dat_file(datFile)
    dat.S1 = tofloat(barData[0]['Диаметр(мм)'])**2 * pi / 4. - \
        tofloat(barData[0].get(
            'ВнутреннийДиаметр', 0))**2 * pi / 4.
    dat.S2 = tofloat(barData[1]['Диаметр(мм)'])**2 * pi / 4. - \
        tofloat(barData[1].get(
            'ВнутреннийДиаметр', 0))**2 * pi / 4.
    dat.E1 = tofloat(barData[0]['МодульУпругости(МПа)'])
    dat.E2 = tofloat(barData[1]['МодульУпругости(МПа)'])
    dat.s = tofloat(expData['Диаметр'])**2 * pi / 4.
    dat.d = tofloat(expData['Диаметр'])
    dat.l = tofloat(expData['Длина'])
    dat.V0 = tofloat(expData['СкоростьУдарника'])
    dat.C1 = tofloat(barData[0]['СкоростьЗвука(мсек)']) * 1e-3
    dat.C2 = tofloat(barData[1]['СкоростьЗвука(мсек)']) * 1e-3
    dat.tarirA = tofloat(expData['КалибровочныйКоэффициентНС'])
    dat.tarirB = tofloat(expData['КалибровочныйКоэффициентОС'])
    t, pp = unpackTable(expData['Осциллограмма'])
#    t, pp = unpackTable(expData['ИмпульсыОбработанные'])
#    sgn = 1.0 if (expData['ТипЭксперимента'] == 't') else -1.0
#    ei = list(sgn * pp[0])
#    er = list(-sgn * pp[1])
#    et = list(sgn * pp[2])
#    n = len(ei)
...
#    bar2 = [0] * n + et
#    nn = 512
#    tt = np.linspace(t[0], t[-1], 1024)
    dt = (t[1] - t[0])
    write_float_rays(datFile, pp[0], pp[1], dt)
